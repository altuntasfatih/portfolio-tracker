defmodule PortfolioTracker.MessageHandlerTest do
  ...
  describe "handle/3 with tracker" do
    ...
    test "it should handle add_stock message",_ do
      assert MessageHandler.handle(:add_stock,["VAKBN", "VAKIF_BANK", "250", "4.5"], @from) == :ok
    end

    test "it should return args parse error",_ do
      assert MessageHandler.handle(:add_stock,["VAKBN", "VAKIF_BANK", "x", "x"], @from) == {:error, :args_parse_error}
    end
    ...
    test "it should handle get message",_ do
      assert MessageHandler.handle(:get,[], @from) == {:ok, "Your Portfolio  \nWorth: 0.0 \nUpdate Time:  \nRate: 0.0 🟢 "}
    end
    ...
    test "it should handle alert messages" do
      ...
      assert MessageHandler.handle(:get_alerts,[], @from) == {:ok, "For TEST lower_limit on 10.67 "}
    end
  ...
  describe "parse/1" do
    test "it should split message into arguments" do
      assert MessageHandler.parse("/add_stock VAKBN VAKIF_BANK 250 4.5") == {:add_stock,["VAKBN", "VAKIF_BANK", "250", "4.5"]}
  ...
  describe "handle_message/1" do
    test "it should return instruction not found" do
      assert MessageHandler.handle_message(%{from: @from,text: nil}) == "Instruction does not exist"
    end
  end
